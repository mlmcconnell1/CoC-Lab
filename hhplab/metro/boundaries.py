...
from hhplab.geo.io import write_geoparquet
...
from hhplab.metro.validate import validate_metro_boundaries
    ...
    *,
    ...
    if joined.empty:
        raise ValueError(
            ...
            f"Check county vintage {county_vintage} and metro definition version {definition_version}."
        )

    dissolved = joined.dissolve(by="metro_id", as_index=False, aggfunc="first")
    dedup_defs = definitions[
        ["metro_id", "metro_name", "definition_version"]
    ...
    return gpd.GeoDataFrame(
        ...
        ].sort_values("metro_id").reset_index(drop=True),
        geometry="geometry",
        crs=counties.crs,
    )
...
def read_metro_boundaries(
    ...
    try:
        ...
        raise FileNotFoundError(
            ...
            f"Run: hhplab generate metro-boundaries --definition-version {definition_version} --counties {county_vintage}"
        ) from None
