...
def select_panel(
    ...
    if config.years:
        ...
        year_candidates = [
            f for f in candidates if config.years.replace("-", "_") in f.name
        ]
        if year_candidates:
    ...
    if selected is None:
        raise FileNotFoundError(
            f"No panel files found matching {PATTERNS['panel']} in {base_dir}"
        )
...
def select_inputs(
    config: BundleConfig, base_dir: Path
...
def build_selection_plan(
    config: BundleConfig, base_dir: Path = Path(".")
) -> SelectionPlan:
    """
    ...
    if "panel" in config.include:
        try:
            panel_path, was_inferred = select_panel(
                config.panel_path, config, base_dir
            )
